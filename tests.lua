...
--SETUP TESTS--
---------------
local mock_party_size = 40
local mock_players_num = 5
...
function GetRaidRosterInfo(idx)
    ...
    local name, rank, sub_group, level, class_loc, class
    if (idx <= mock_players_num) then
        name = CLASS_TO_TEST_REF .. idx
        class = CLASS_TO_TEST
        sub_group = group
    else
        name = "NotA" .. CLASS_TO_TEST_REF
        class = "!" .. CLASS_TO_TEST
        sub_group = group
    end
    return name, rank, sub_group, level, class_loc, class
end
